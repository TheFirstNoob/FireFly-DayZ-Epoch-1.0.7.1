...
"PVDZ_plr_SwitchMove" addPublicVariableEventHandler {((_this select 1) select 0) switchMove ((_this select 1) select 1);};
...
"PVDZ_plr_Delete" addPublicVariableEventHandler {(_this select 1) setVariable ["sched_co_fliesDeleted", true];(_this select 1) spawn sched_co_deleteVehicle};
...
"PVDZE_veh_Publish2" addPublicVariableEventHandler {(_this select 1) call server_publishVeh2};
...
"PVDZ_sec_atp" addPublicVariableEventHandler {
	...
	call {
		if (typeName _y == "STRING") exitwith
		{
			diag_log _y;
		};

		if (count _y == 2) exitwith
		{
...
"PVDZ_objgather_Knockdown" addPublicVariableEventHandler {
	...
	if (_dis < 30 && {_treeModel in dayz_trees || (_treeModel in dayz_plant)} && {_uid != ""}) then
	{
		_tree setDamage 1;
