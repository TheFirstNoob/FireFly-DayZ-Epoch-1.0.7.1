...
// Добавим защиту для отправки PVDZ_Send с повышенным риском
if (_variable in ["Unconscious","DragPlayer","DropPlayer","Legs","RemoveObject","CarryPlayer","LoadWounded","UnloadWounded"]) then
{
	...
	//diag_log text format ["[СЕРВЕР]: [server_sendToClient.sqf]: Игрок [%1:%2] Использовал функцию %3 на/в %4.",(name _activatingPlayer),_playerUID,_variable,_objPos];
};

if (_exitReason != "") exitWith {diag_log _exitReason};

//diag_log format ["[СЕРВЕР]: [server_sendToClient.sqf]: %1, %2, %3, %4", _unit, _variable, _arraytosend, _owner];
