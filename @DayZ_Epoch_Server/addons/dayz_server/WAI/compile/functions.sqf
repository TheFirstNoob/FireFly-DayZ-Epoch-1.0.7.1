WAI_GetTurrets 	=
{
	// Переадаптировано от KillZone Kid's KK_fnc_commonTurrets для Arma3.
	local _array 	= 	[];
...
WAI_FindAmmo 	=
{
	...
	if (_result == "") then
	{
		diag_log format["[WAI]: [function.sqf]: Невозможно найти магазин для оружия: %1.", _weapon];
	};
	_result
};
	...
	call {
		...
		if (WAI_MessageType == "DynamicText") exitWith
		{
			local _color 	= 	call {
				...
				if(_type == "Random") exitWith {"#708090"};
				"#3339FF";
			};

			local _params 	= 	["0.50","#FFFFFF","0.65",_color,0,-.35,10,0.5];
...
WAI_AutoClaimAlert 	=
{
	...
	local _name 	= 	if (typeName _unit == "ARRAY") then
						{
							_unit select 1;
						}
						else
...
WAI_CleanUp 	=
{
		...
		if (WAI_CleanMissionTime > 0 || _time0ut) then
		{
			if ((diag_tickTime - _time) > (WAI_CleanMissionTime * 60) || _time0ut) then
			{
				...
				if (WAI_CleanMissionCrate || _time0ut) then
				{
					...
					{
						// Ждем пока игроки не окажутся на расстоянии не менее 100 метров.
						if !([_position,100] call isNearPlayer) then
						{
							{
	...
	diag_log format ["[WAI]: [function.sqf]: Очистка миссии %1 выполнена.",_mission];
};
...
WAI_GenerateVehKey 	=
{
	...
	if !(_isKeyOK) exitWith
	{
		...
		diag_log format["[WAI]: [function.sqf]: Ошибка генерации ключа для техники: %1 | Миссия: %2",_vehicle,_mission];
	};
	...
	if (WAI_VehKeys == "KeyonAI") exitWith
	{
		...
		{
			diag_log format["[WAI]: [ОТЛАДКА]: [function.sqf]: Есть %1 мертвых ИИ для миссии %2 чтобы выдать ключ от машины",_ailist,_mission];
			diag_log format["[WAI]: [ОТЛАДКА]: [function.sqf]: Ключ был добавлен ИИ: %1 | Для техники: %2",_unit,_vehicle];
		};
	};
};
...
WAI_CleanAircraft =
{
	...
	if (WAI_DebugMode) then
	{
		diag_log "[WAI]: [ОТЛАДКА]: [function.sqf]: Самолет очищен";
	};
};
...
{
	{
		if !(_x getVariable ["DoNotFreeze", false]) then
		{
			...
			if (WAI_DebugMode) then
			{
				diag_log format ["[WAI]: [ОТЛАДКА]: [function.sqf]: Заморожено ИИ Групп: %1", _x];
			};
		};
	} count _this;
};
...
WAI_UnFreeze = {
	{
		if !(_x getVariable ["DoNotFreeze", false]) then {
			...
			if (WAI_DebugMode) then
			{
				diag_log format ["[WAI]: [ОТЛАДКА]: [function.sqf]: Разморожено ИИ Групп: %1", _x];
			};
		};
	} count _this;
};
