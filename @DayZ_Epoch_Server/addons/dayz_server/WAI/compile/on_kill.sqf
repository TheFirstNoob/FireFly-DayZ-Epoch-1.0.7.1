...
_unit setVariable ["bodyName","NPC",false];

if (WAI_HasMoney && Z_singleCurrency) then
{
	if (WAI_RandomMoney) then
	{
		_unit setVariable ["cashMoney", round(random WAI_MoneyCount) , true];
	}
	else
	{
		_unit setVariable ["cashMoney", WAI_MoneyCount , true];
	};
};
...
if (isPlayer _player) then
{
	...
	if (WAI_HumanityGain) then
	{
		...
		{
			_player setVariable ["humanity",(_humanity - _gain),true];
		};

		if (_unit getVariable ["Bandit", false]) then
		{
			_player setVariable ["humanity",(_humanity + _gain),true];
		};					
	};
...
if (_unit hasWeapon "NVGoggles_DZE" && {floor(random 100) < 20}) then
{
	_unit removeWeapon "NVGoggles_DZE";
};