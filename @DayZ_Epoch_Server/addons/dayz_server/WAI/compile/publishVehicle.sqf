...
if (!_position_fixed) then
{
	...
	while {count _vehpos > 2} do { 
		_vehpos 		= 	[_position,12,_max_distance,10,0,0.7,0] call BIS_fnc_findSafePos;
...
_vehicle setVariable ["CharacterID","1",true];
...
{
	diag_log format["[WAI]: [publishVehicle.sqf]: [ОТЛАДКА]: Создан: %1 на позиции: %2",_class,_vehpos];
};
...
{
	{
		...
		if (!(["glass", _selection] call fnc_inString) && {_dam > 0.1}) then
		{
			...
			if (WAI_DebugMode) then
			{
				diag_log format ["[WAI]: [publishVehicle.sqf]: [ОТЛАДКА]: Состояние для: %1 на: %2", _selection, _dam];
			};
		};
	...
	if (WAI_DebugMode) then
	{
		diag_log format["[WAI]: [publishVehicle.sqf]: [ОТЛАДКА]: Добавлено топлива %1 для техники",_fuel];
	};
};
...
if (WAI_KeepVeh) then
{
	_vehicle addEventHandler ["GetIn", {
		...
		local _array 		= 	[];
		{
			local _hit 			= 	([_vehicle,_x] call object_getHit) select 0;
			...
			if (WAI_DebugMode) then
			{
				diag_log format ["[WAI]: [publishVehicle.sqf]: [ОТЛАДКА]: Выбрана деталь: %1, Урон: %2",_selection,_hit];
			};
		} count (_vehicle call vehicle_getHitpoints);
		...
		if ((_result select 0) != "PASS") then
		{
			...
			diag_log format ["[WAI]: [publishVehicle.sqf]: [ОШИБКА]: Ошибка получения ID для %1: UID %2",_class, _uid];
		}
		else
		{
			...
			diag_log ("[WAI]: [publishVehicle.sqf]: [ПУБЛИКАЦИЯ]: Создано " + _class + " с ID" + str(_uid));

			if (WAI_VehMessage) then
			{
				[nil,(_this select 2),"loc",rTitleText,"Эта техника была СОХРАНЕНА в Базе Данных.","PLAIN",5] call RE;
			};
		};
	}];
}
else
{
	_vehicle addEventHandler ["GetIn", {
		...
		if (WAI_VehMessage) then
		{
			[nil,(_this select 2),"loc",rTitleText,"ВНИМАНИЕ: ЭТА ТЕХНИКА НЕ СОХРАНЕНА И БУДЕТ УДАЛЕНА ПОСЛЕ РЕСТАРТА!","PLAIN",5] call RE;
		};
		...
		//diag_log format ["[WAI]: [publishVehicle.sqf]: [HandleDamage]: Установлено HandleDamage EV для %1", (typeOf _vehicle)]; 	// Для тестов.
	}];
};
