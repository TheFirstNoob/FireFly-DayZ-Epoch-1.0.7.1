...
while {!_complete} do {
	...
	if (diag_tickTime - _starttime >= (WAI_Timeout * 60) && !_playerNear) then
	{
		...
		diag_log format["[WAI]: [missionMonitor.sqf]: Время вышло для миссии: %1 | %2",_name,_position];
	}
	else
	{
		if (_playerNear) then
	...
	if ([_mission,_completionType,_killpercent,_position] call WAI_CompletionCheck || count _unitGroups == 0) then
	{
		...
		diag_log format["[WAI]: [missionMonitor.sqf]: Миссия выполнена: %1 | %2",_name,_position];
	};
