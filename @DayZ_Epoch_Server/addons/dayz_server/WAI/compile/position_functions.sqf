	...
	while {!_validspot && {_count < (count _positions)}} do {
		...
		{
			{
				if ((typeName _x) == "ARRAY" && {_position distance _x < WAI_AvoidMissions}) exitWith
				{
					...
					{
						diag_log format ["[WAI]: [position_functions.sqf]: [ОТЛАДКА]: Недопустимая позиция: %1", _x];
					};
					_validspot 	= 	false;
				};
		...
		if (WAI_AvoidPlayers != 0) then
		{
			if ([_position, WAI_AvoidPlayers] call isNearPlayer) then
			{
				if (WAI_DebugMode) then
				{
					diag_log "[WAI]: [position_functions.sqf]: [ОТЛАДКА]: Недопустимая позиция: Недопустимая позиция: (Игрок)";
				};
				_validspot 	= 	false;
			};
		};
		...
		if (_validspot) then
		{
			if (WAI_DebugMode) then
			{
				diag_log("[WAI]: [position_functions.sqf]: [ОТЛАДКА]: Позиция найдена:" + str(_position));
			};
		};
