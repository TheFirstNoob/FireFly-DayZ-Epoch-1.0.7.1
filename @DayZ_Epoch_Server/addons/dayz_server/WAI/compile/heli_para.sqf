...
{
	diag_log "[WAI]: [ОТЛАДКА]: [heli_para.sqf]: Десант ждет игрока...";
};

// Ждем пока игрок не окажется на расстоянии срабатывания триггера или время миссии не истечет..
...
if (!_missionrunning) exitWith
{
	if (WAI_DebugMode) then
	{
		diag_log format["[WAI]: [ОТЛАДКА]: [heli_para.sqf]: Миссия %1 уже завершена. Отменяем десант.",_position];
	};
};
...
if (WAI_DebugMode) then
{
	diag_log format ["[WAI]: [ОТЛАДКА]: [heli_para.sqf]: Создаем %1 с %2 ИИ для Десанта на: %3",_heliClass,_paranumber,_position];
};
...
// Это случайное число используется для запуска вертолета на случайном расстоянии от миссии.
...
if (WAI_DebugMode) then
{
	...
	diag_log format["[WAI]: [heli_para.sqf]: Десантная группа готова и вылетела на миссию: %1",_diag_distance];
};
...

{
	...
	if (_hero) then
	{
		_x setVariable ["Hero", true];
		_x setVariable ["humanity", WAI_RemoveHumanity];
	};

	if (_bandit) then
	{
		_x setVariable ["Bandit", true];
		_x setVariable ["humanity", WAI_AddHumanity];
	};
	...
	if (_heliPos distance [(_position select 0),(_position select 1),100] <= _starttodrop) then
	{
		...
		for "_x" from 1 to _paranumber do {
			...
			if (!_unarmed) then
			{
				...
				{
					diag_log text format ["[WAI]: [ОШИБКА]: [heli_para.sqf]: КлассНейм оружия (%1) неверен!",_weapon];

					_weapon 	= 	"M4A1_DZ"; 	// Заменим на то, что разрешено и работает.
				};
		...
		{
			{
				_x setVariable ["Hero",true,false];
				_x setVariable ["humanity", WAI_RemoveHumanity];
			} count (units _pgroup);
		};
		...
		{
			{
				_x setVariable ["Bandit",true,false];
				_x setVariable ["humanity", WAI_AddHumanity];
			} count (units _pgroup);
		};
		...
		if (WAI_DebugMode) then
		{
			diag_log format ["[WAI]: [ОТЛАДКА]: [heli_para.sqf]: Создано %1 ИИ для Десанта",_paranumber];
		};
...
else
{
	...
	while {_cleanheli} do {
		...
		if (((getPos _helicopter) distance [(_startPos select 0),(_startPos select 1),100] <= 2000) || (!alive _helicopter)) then
		{
			...
			if (WAI_DebugMode) then
			{
				diag_log "[WAI]: [ОТЛАДКА]: [heli_para.sqf]: Вертолет десанта удален";
			};
