...
{
	diag_log format["[WAI]: [ОТЛАДКА]: [patrol_winorfail.sqf]: Запускаю миссию номер: %1",_mission];
};
...
[_position,_mission,_name,_completionType,_color,_showMarker,_msgwin,_msglose,_missionType,_difficulty,_countWP] spawn {
	...
	while {!_timeout && !_complete} do {
		...
		if (_showMarker && {_startMarker}) then
		{
			if (ai_show_count) then
			{
				...
				_text 		= 	format["%1 (%2 ИИ)",_name,_aiCount];
			}
			else
			{
				_text 	= 	_name;
		...
		if (!_disabled && (!(canMove _vehicle) || !(alive _leader))) the
		{
			_horb 	= 	if (_missionType == "MainHero") then
						{
							"Hero"
						}
						else
	...
	if (_complete) then
	{
		...
		diag_log format["[WAI]: [МИССИЯ: %1]: Завершена на: %2",_name,_position];
		...
		if (wai_clean_mission_time > 0) then
		{
			...
			while {!_cleaned} do {
				...
				if (diag_tickTime - _finish_time >= 60*wai_clean_mission_time) then
				{
					...
					if (!isNil "_unitGroup2") then
					{
						deleteGroup _unitGroup2;
					};
	...
	if (_timeout) then
	{
		...
		diag_log format["[WAI]: [МИССИЯ: %1]: Время вышло на: %2",_name,_position];
	};
};