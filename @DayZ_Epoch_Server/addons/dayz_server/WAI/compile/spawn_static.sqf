...

{
	...
	{
		_class 	= 	WAI_StaticWeapons call BIS_fnc_selectRandom;
	};
	...
	if (WAI_StaticUseWeapon) then
	{
		...
		if (!_unarmed) then
		{
			...
			{
				diag_log text format ["[WAI]: [spawn_static.sqf]: [ОШИБКА]: КлассНейм оружия (%1) неверен!",_weapon];
				_weapon 	= 	"M4A1_DZ";
			};
...
{
	diag_log format ["[WAI]: [spawn_static.sqf]: [ОТЛАДКА]: Создано в %1 %2",(count _position),_class];
};