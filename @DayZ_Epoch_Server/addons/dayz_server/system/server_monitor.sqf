...
{
	call compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_plantSpawner.sqf"; 	// Рисуем Псевдослучайные сид
};
...
execFSM "\z\addons\dayz_server\system\server_weather.fsm";
...
if (_hiveLoaded) then
{
	_serverVehicleCounter spawn {
		...
		if (_vehLimit > 0) then
		{
			diag_log ("[БАЗА ДАННЫХ]: [server_monitor.sqf]: Создано Техники: " + str(_vehLimit));
