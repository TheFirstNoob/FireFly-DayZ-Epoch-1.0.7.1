...
#ifdef OBJECT_DEBUG
	diag_log ("[СЕРВЕР]: [server_publishVehicle.sqf]: [ПУБЛИКАЦИЯ]: Попытка " + str(_object));
#endif
...
if (_outcome != "PASS") then
{
	...
	diag_log("[СЕРВЕР]: [server_publishVehicle.sqf]: Ошибка получения ID для: " + str(_uid));
}
else
{
	...
	#ifdef OBJECT_DEBUG
		diag_log("[СЕРВЕР]: [server_publishVehicle.sqf]: Выбрано: " + str(_oid));
	#endif

	_object setVariable ["lastUpdate",diag_tickTime];
	_object setVariable ["CharacterID", _characterID];
	...
	diag_log ("[СЕРВЕР]: [server_publishVehicle.sqf]: [ПУБЛИКАЦИЯ]: Создано " + (_class) + " с ID " + str(_uid));
};