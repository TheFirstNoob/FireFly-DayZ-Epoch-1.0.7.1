...
server_obj_pos =
{
	...
	#ifdef OBJECT_DEBUG
		diag_log ("[БАЗА ДАННЫХ]: [updateObject_functions.sqf]: ЗАПИСЬ: "+ str(_key));
	#endif
};
...
server_obj_killed =
{
	...
	local _exitReason = call {
		// Нельзя использовать владельца, потому что игрок может быть уже мертв, нельзя использовать расстояние, потому что игрок может быть далеко от места крушения автомобиля
