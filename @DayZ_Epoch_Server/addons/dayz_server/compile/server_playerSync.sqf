...
local _exitReason = call {
	...
	{
		format["[СЕРВЕР]: [server_playerSync.sqf]: [СИНХРОНИЗАЦИЯ]: Невозможно синхронизировать игрока %1 рядом с respawn_west %2. Это нормально при Перезаходе или при Переодевании.",_name,_charPos]
	};
	...
	if (count _magTemp > 0) then
	{
		_magazines = [(magazines _character),20] call array_reduceSize;
		{
			...
			if (_class in _magazines) then
			{
				local _MatchedCount 	=
				{
					local _compare 	= 	if (typeName _x == "ARRAY") then
										{
											_x select 0;
										}
										else
...
// Проверяем рюкзак игрока каждый раз при синхронизации
...
// Если игрок в технике, то обновляем позицию игрока
...
{ 	// Вызывается их player_onDisconnect
	if (_this select 4) then
	{ 	// В РЕЖИМЕ БОЯ
		_medical set [1, true]; 	// Установим бессознание на true
		_medical set [10, 150]; 	// Таймер
	};
	...
	if (_isInVehicle) then
	{
		...
		if (_relocate) then
		{
			...
			while {_newPos distance _charPos == 0} do {
				...
				// Максимум 4 км от недопустимой позиции (нужно закончить быстро, чтобы server_playerSync работал ниже)
...
if (_onLadder || _isInVehicle || _isTerminal) then
{
	...
	// Если позицию нужно обновить, то убедимся, что она находится на уровне земли!
...
// Принудительно обновим содержимое у техники/палаток
{
