/*
	changeCode - Сейфы, ЛокБоксы и Дверти скрипт смены кода
...
{
	diag_log "[СЕРВЕР]: [server_changeCode.sqf]: [ОШИБКА]: Неверно получены данные. Параметров получено меньше, должно быть 4!";
};
...
{
	diag_log "[СЕРВЕР]: [server_changeCode.sqf]: [ОШИБКА]: Недопустимый объект хранилища!"
};
...
{
	diag_log "[СЕРВЕР]: [server_changeCode.sqf]: [ОШИБКА]: Запрос с Нуль _player объект";
};

if (isNull _object) exitWith
{
	diag_log format["[СЕРВЕР]: [server_changeCode.sqf]: [ОШИБКА]: Запрос с Нуль объект от: %1 (%2).",_name,_playerUID];
};
...
_ownerPUID 		= 	if (_charID == "0000" || _charID == "10000") then
					{
						_playerUID
					}
					else
...
_message 	= 	format ["[СЕРВЕР]: [server_changeCode.sqf]: [СМЕНА КОДА]: Игрок: %1 (%2) изменил код для: %3 на: %4 @%5 %6",_name,_playerUID,_lockedClass,_characterID,mapGridPosition _player,_pos];
diag_log _message;