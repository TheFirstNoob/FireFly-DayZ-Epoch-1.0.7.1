...
_playerUID 	= 	if (count _this > 3) then
				{
					_this select 3
				}
				else
...
_name 			= 	if (alive _player) then
					{
						[(name _player),1] call fnc_sanitizeInput;
					}
					else
					{
						"Неизвестный игрок";
...
if (_VGobjID == "0") then
{
	_VGobjID = (toString (18 call VG_RandomizeMyKey));
}
else
{
	...
	{
		diag_log format["[СЕРВЕР]: [server_storeVehicle.sqf]: [Виртуальный гараж]: [ОШИБКА]: Невозможно найти технику в VGobjUID = %1 в массиве vg_alreadySpawned. Возможно дублирование техники в гараже. PlayerUID: %2", _VGobjID, (getPlayerUID _player)];
	};
};
...

{
	...
	if (_damage > 0) then
	{
		_array set [count _array,[_selection,_damage]]
	};
} count _hitpoints;
...
_message = format["[СЕРВЕР]: [server_storeVehicle.sqf]: [Виртуальный гараж]: %1 (%2) сохранен в гараж %3 @%4 %5",_name,_playerUID,_class,mapGridPosition _player,getPosATL _player];
diag_log _message;