/*
	Maintain Area - Автор: Skaronator 
	Обновлен: iben
...
if ((_this select 1) == 1) exitWith
{
    ...
    diag_log format [
         "[СЕРВЕР]: [server_maintainArea.sqf]: [MAINTAIN AREA]: Игрок: %1 - %2. Объектов: %3, Позиция: %4."
        ,if (alive _player) then {name _player} else {"Игрок мертв"}
