...
{
	diag_log format ["[СЕРВЕР]: [server_swapObject.sqf]: [ОШИБКА]: Неверно получены данные. Параметров получено меньше, должно быть 8 от игрока: %1",_this select 5];
};
...
if (!_allowed || !_proceed) exitWith
{
	...
	diag_log ("[СЕРВЕР]: [server_swapObject.sqf]: Неверная замена объекта от playerUID:" + _playerUID);
};
...
[
	...
	,"TentStorageWinter3"
	,"TentStorageWinter4"
	,"WinterDomeTentStorage0"
	,"WinterDomeTentStorage1"
	,"WinterDomeTentStorage2"
	,"WinterDomeTentStorage3"
	,"WinterDomeTentStorage4"
];
...
// Не используйте server_updateObject, сейфы и локбоксы требуются в другой обработке
...
diag_log format["[СЕРВЕР]: [server_swapObject.sqf]: [ПУБЛИКАЦИЯ]: Игрок %1(%2) улучшил или ухудшил объект до %3 с UID:%4 @%5",(_activatingPlayer call fa_plr2str),_playerUID,_class,_uid,((_worldspace select 1) call fa_coor2str)];