...
{
	diag_log "[СЕРВЕР]: [Server_PublishVehicle2.sqf]: [ОШИБКА]: Неверно получены данные. Параметров получено меньше, должно быть 6!";
};
...
{
	diag_log("[СЕРВЕР]: [Server_PublishVehicle2.sqf]: Ошибка получения ID для: " + str(_uid));
}
else
{
	...
	#ifdef OBJECT_DEBUG
		diag_log("[СЕРВЕР]: [Server_PublishVehicle2.sqf]: Выбрано:" + str(_oid));
	#endif
	...
	if (DZE_TRADER_SPAWNMODE) then
	{
		...
		// Не используйте setPos или CAN_COLLIDE здесь. Он будет появляться внутри других транспортных средств.
	...
	_object setVariable ["CharacterID", _characterID,[true,false] select (_donotusekey)];
	...
	diag_log format["[СЕРВЕР]: [Server_PublishVehicle2.sqf]: [ПУБЛИКАЦИЯ]: Игрок: %1(%2) купил: %3 с UID: %4 @%5",(_activatingPlayer call fa_plr2str),_playerUID,_class,_uid,(_location call fa_coor2str)];
};
