...
_playerName 	= 	toString (_this select 4); 	// Отправим в массив чтобы избежать publicVariable кик Battleye фильтра
...
// Сообщения о смерти
_suicide 	= 	((_sourceName == _playerName) || (_method == "suicide"));
...
{
	// Нет имени у источника, Дистанция или оружие нужны для: "%1 умер от %2" str_death_%1 (Смотри stringtable)
...
if (_playerName != "unknown" || {_sourceName != "unknown"}) then
{
	...
	_message = call {
		if (_type == "died") exitwith {format ["%1(%3) умер от: %2", _bodyName, localize format["str_death_%1",_message select 2],_playerID]};
		if (_type == "killed") exitwith {format ["%1(%5) был убит от рук: %2(%6), (С оружия: %3),(Расстояние: %4м)", _bodyName, _message select 2, _message select 3, _message select 4,_playerID,_sourceID]};
