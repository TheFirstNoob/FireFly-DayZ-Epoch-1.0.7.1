...
_unitGroup 	= 	if (isNull (_this select 1)) then
				{
					[] call DZAI_createGroup
				}
				else
				{
					_this select 1
...
for "_i" from 1 to _totalAI do {
	...
	{
		_unit addEventHandler ["Fired", {_this call DZAI_aiFired;}];
	};

	if (DZAI_debugLevel > 1) then
	{
		diag_log format["[DZAI]: [fn_spawnGroup.sqf]: [РАСШИРЕННАЯ ОТЛАДКА]: Создано ИИ тип: %1 с weapongrade: %2 для группы: %3.",_type,_weapongrade,_unitGroup];
	};
};
...
if (!isNil "_dummy") then
{
	...
	if (DZAI_debugLevel > 1) then
	{
		diag_log format["[DZAI]: [fn_spawnGroup.sqf]: [РАСШИРЕННАЯ ОТЛАДКА]: Удалено 1 пустышка ИИ для группы: %1.",_unitGroup];
	};
};
