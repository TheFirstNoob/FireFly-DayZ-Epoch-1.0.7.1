...
if ((typeName _spawnMarker) != "STRING") exitWith
{
	diag_log "[DZAI]: [fn_customSpawnUnits.sqf]: [ОШИБКА]: Строка маркера не указана (!=STRING)"
};
_totalAI 		= 	if ((typeName (_this select 1)) == "SCALAR") then
					{
						_this select 1
					}
					else
...
_weapongrade 	= 	if ((typeName (_this select 2)) == "SCALAR") then
					{
						_this select 2
					}
					else
...
_respawn 		= 	if ((count _this) > 3) then
					{
...
_respawnTime 	= 	if ((count _this) > 4) then
					{
...
{
	_trigger setVariable ["respawnTime",_respawnTime];
};
...
{
	diag_log format ["[DZAI]: [fn_customSpawnUnits.sqf]: [ОШИБКА]: Создана Кастомная позиция создания ИИ %1 на %2 с кол-вом %3 ИИ, weapongrade %4, Создано %5, Время возрождения %6.",_spawnMarker,mapGridPosition _trigger,_totalAI,_weapongrade,_respawn,_respawnTime];
};
