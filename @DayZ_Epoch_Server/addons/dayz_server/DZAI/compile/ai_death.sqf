...
_deathType 	= 	if ((count _this) > 2) then
				{
					_this select 2
				}
				else
...
// Проверим количество живых ИИ, Сразу сохраним группу если она пуста.
_unitsAlive 	= 	({alive _x} count (units _unitGroup));
_groupIsEmpty 	= 	if (_unitsAlive == 0) then
					{
						_unitGroup call DZAI_protectGroup; true
					}
					else
					{
						false
...
// Обновим счетчик размера группы
_groupSize 	= 	(_unitGroup getVariable ["GroupSize",0]);
if (_groupSize > 0) then
{
	_unitGroup setVariable ["GroupSize",(_groupSize - 1)]
};
...
if !(isNull _victim) then
{
	...
	if ((combatMode _unitGroup) == "BLUE") then
	{
