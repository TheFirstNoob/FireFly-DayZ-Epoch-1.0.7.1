...
server_deleteObjDirect 		= 	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_deleteObjDirect.sqf";
...
{
	server_publishVeh3 	= 	compile preprocessFileLineNumbers "\z\addons\dayz_server\compile\server_publishVehicle3_vkc.sqf";
}
else
{
...
server_hiveWrite 	=
{
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions.sqf]: ПОПЫТКА ЗАПИСИ: " + _this);
	local _data 	= 	"HiveExt" callExtension _this;
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions.sqf]: ЗАПИСЬ: " +str(_data));
};

server_hiveReadWrite 	=
{
	local _key 			= 	_this;
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions.sqf]: ПОПЫТКА ЧТЕНИЯ/ЗАПИСИ: " + _key);
	local _data 		= 	"HiveExt" callExtension _key;
	//diag_log ("[БАЗА ДАННЫХ]: [server_functions.sqf]: ЧТЕНИЯ/ЗАПИСИ: " +str(_data));
...
server_getStatsDiff 	=
{
...

	{
		...
		_statsArray set [_forEachIndex, _new];
...
fa_plr2str 	=
{
	...
	if (!isNil "_y") then
	{
		...
		if ((isNil "_name" || {(_name == "")}) && ({alive _y})) then
		{
			_name 	= 	name _y;
		};
