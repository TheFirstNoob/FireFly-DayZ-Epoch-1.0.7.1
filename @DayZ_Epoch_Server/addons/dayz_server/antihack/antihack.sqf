...
diag_log (_diag_prefix + "Инициализация Epoch Antihack/Admin Tools...");
...
diag_log (_diag_prefix + "Конфигурация успешно загружена!");
...
diag_log (_diag_prefix + "Список Администрации успешно загружен!");
...
local _AH_Global = ("
	...
	"+_kfc_msg+" = {
		...
		[format['Функция общения: Получена неопределенная опция: %1', _opt], 1] call "+_kfc_msg+";
	};
...
if (_safezones) then {_AH_Global = _AH_Global + ("
	...
	"+_kfc_sz1+" = {
		...
		local _msg = 'Вы вошли в Торговую зону!';
		taskHint [_msg, [0,1,0,1], 'taskDone']; [_msg, 2] call "+_kfc_msg+";

		['LOG', [format['Вход в Торговую зону @ %1', mapGridPosition player], 4]] call "+_kfc_pvs+";
		if ([player, 'inCombat', false] call "+_kfc_gvar+") then {
			['LOG', [format['В БОЮ при входе в Торговую зону @ %1', mapGridPosition player], 4]] call "+_kfc_pvs+";
			sz_combatcnt = sz_combatcnt + 1;
			if (sz_combatcnt >= 3) then {
				['LOG', [format['Абуз Торговой зоны @ %1. Вход в Торговую зону В БОЮ: %2', mapGridPosition player, sz_combatcnt], 4]] call "+_kfc_pvs+";
			};
		};");
				...
				if (vehicle player == player) then {
					...
					if ((player distance _ct <= _dis) && {_ct isKindOf 'Air' || {_ct isKindOf 'LandVehicle'} || {_ct isKindOf 'Ship'}}) then {
						...
						if (_deny) then {
							...
							['Вы не можете открыть Инвентарь техники т.к. она не принаджелит вам!', 2] call "+_kfc_msg+";
						};
					};
				};
		...
		_AH_Global = _AH_Global + ("
			...
			player_fired = {
				...
				cutText ['Вы не можете стрелять в Торговой зоне!', 'WHITE IN'];
			};
	...
	"+_kfc_sz2+" = {
		...
		local _msg = 'Вы вышли из Торговой зоны!';
		taskHint [_msg, [1,0,0,1], 'taskFailed']; [_msg, 2] call "+_kfc_msg+";

		local _time = diag_tickTime - sz_time;
		['LOG', [format['Вышел из Торговой зоны @ %1 после %2 минут(ы)', mapGridPosition player, round(_time/60)], 4]] call "+_kfc_pvs+";
		...
		local _veh = vehicle player;"); if (_timer > 0) then {_AH_Global = _AH_Global + ("
			if (_time >= 60) then {
				...
				{
					[[format['Защита закончится через %1 секунд!', _x], 1], 3] call "+_kfc_msg+";
					uiSleep 1;
				...
				if (_speedlimit > 0) then {_AH_Global = _AH_Global + ("
					if !(_veh isKindOf 'Air') then {
						...
						if (_speed > "+str _speedlimit+") then {
							...
							local _mod = if (_speed > 50) then {0.1} else {0.6};
	...
	comment 'Торговые зоны получают свой собственный поток из-за uiSleep, необходимого для таймера выхода.';
	_szqueue spawn {
...
_AH_Global = _AH_Global + ("
	'"+_kfc_rekey+"' addPublicVariableEventHandler {
		...
		if (dayz_authKey != toString(_in select 3)) exitWith {
			['LOG', ['RE получил неверный ключ клиента! Возможно Читер зашел на сервер!', 3]] call "+_kfc_pvs+";
		};
		...
		if (_id == 'TAR_TP2U') exitWith {
			...
			[format['Вы были телепортированы к %1 Администратором ""%2.""', mapGridPosition player, _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_HEAL') exitWith {
			...
			[format['Вы были вылечены Администратором ""%1.""', _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'MAG_GIVE') exitWith {
			for '_x' from 1 to (_param select 1) do {player addMagazine (_param select 2)};
			call player_forceSave;
			[format['Вам выдали %1 ""%2"" Администратором ""%3.""', _param select 1, _param select 2, _info], 4] call "+_kfc_msg+";
		};
		...
		if (_id == 'BP_GIVE') exitWith {
			...
			call player_forceSave;
			[format['Вам выдали Рюкзак ""%1"" Администратором ""%2.""', _param select 1, _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_BANK') exitWith {
			player setVariable ["+str _bankvar+", (player getVariable ["+str _bankvar+", 0]) + (_param select 1), true];
			call player_forceSave;
			[format['%1 %2 было добавлено в ваш Банк Администратором ""%3.""', [_param select 1] call BIS_fnc_numberText, CurrencyName, _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_WALLET') exitWith {
			player setVariable ["+str _cashvar+", (player getVariable ["+str _cashvar+", 0]) + (_param select 1), true];
			call player_forceSave;
			[format['%1 %2 было добавлено в ваш Кошелек Администратором ""%3.""', [_param select 1] call BIS_fnc_numberText, CurrencyName, _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_HUMANITY') exitWith {
			player setVariable ['humanity', (player getVariable ['humanity', 0]) + (_param select 1), true];
			call player_forceSave;
			[format['Вам выдано %1 человечности Администратором ""%2.""', [_param select 1] call BIS_fnc_numberText, _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_AMMO') exitWith {
			...
			[format['Вам выдано %1 патронов для вашего текущего оружия Администратором ""%2.""', _param select 1, _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_MOVINVEH') exitWith {
			player moveInCargo (vehicle _sender);
			[format['Вы были перемещены в Технику Администратором ""%1.""', _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_REPAIR') exitWith {
			...
			[format['Ваша Техника была починена Администратором ""%1.""', _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_FREEZE') exitWith {
			if ((_param select 1) == 0) then {disableUserInput false} else {disableUserInput true};
			[format['Вы были %1 Администратором ""%2.""', if ((_param select 1) == 0) then {'Разморожены'} else {'Заморожены'}, _info], 4] call "+_kfc_msg+";
		};
		...
		if (_id == 'TAR_RMGEAR') exitWith {
			...
			call player_forceSave;
			[format['Ваш инвентарь был очищен Администратором ""%1.""', _info], 4] call "+_kfc_msg+";
		};
		...
		if (_id == 'TAR_SENDOCEAN') exitWith {
			player setPos [0,0,0];
			[format['Вы были отправлены в Океан Администратором ""%1.""', _info], 4] call "+_kfc_msg+";
		};
		if (_id == 'TAR_EJECT') exitWith {
			player action ['Eject', vehicle player];
			[format['Вы были высажены с Техники Администратором ""%1.""', _info], 4] call "+_kfc_msg+";
		};
...
_AH_Global = _AH_Global + ("
	_queue set [count _queue, [{
		...
		if (_puid in "+str _chatbanned+") exitWith {
			...
			['Вам заблокировали возможность писать в чат!', 2] call "+_kfc_msg+";
		};
		...
		if (_txt in "+str _badChat+" && {!((name player + _puid) in "+str (_sorted select 4)+")} && {!(_puid in "+str (_sorted select 4)+")}) then {
			_display closeDisplay 0; ['LOG', [format['Запрещенная команда: %1', _txt], 9]] call "+_kfc_pvs+";
		};"); if ("day/night" in _chatcmds) then {_AH_Global = _AH_Global + ("
		if (_txt in ['!day', '!night', '/day', '/night']) then {
			_display closeDisplay 0;
			if (count playableUnits < "+str _voteplayers+") exitWith {[format['Чтобы провести голосование, на сервере должно быть %1 игроков.', "+str _voteplayers+"], 2] call "+_kfc_msg+"};
...
_AH_Global = _AH_Global + ("
	{
		...
		if (PVCDZ_plr_Login select 8) then {['LOG', [format['Новый игрок: %1 (%2)', name player, getPlayerUID player], 10]] call "+_kfc_pvs+"};
...
if (_bfs) then {
	...
	_AH_Normal = _AH_Normal + ("
		{
			preprocessFile 'Анализ завершен. Запрещенное';
			{
				local _file = preprocessFile _x;
				if (_file != '') exitWith {format['Запрещенный файл: %1 - Содержимое: %2', _x, _file] call "+_kfc_ban+"};
			} count "+str _badFiles+";
		} spawn {call _this};
	");
};
		...
		_queue set [count _queue, [{
			{
				if (!isNil _x) exitWith {
					format['Запрещенная переменная: %1', _x] call "+_kfc_ban+";
				};
			} count "+str _badVars+";
		}, 5, diag_tickTime]];
	");
...
if (_oef) then {
	...
	_AH_Normal = _AH_Normal + ("
		...
		_queue set [count _queue, [{
			if ("+_kfc_oef+") then {"+_kfc_oef+" = false} else {'onEachFrame модицифирован' call "+_kfc_ban+"};
		}, 13, diag_tickTime]];
	");
};
if (_bdc) then {
		...
		_queue set [count _queue, [{
			{
				if (!isNull findDisplay _x) exitWith {
					format['Запрещенный findDisplay: %1', _x] call "+_kfc_ban+";
				};
		...
		_queue set [count _queue, [{
			local _type = typeOf player;
			{
				if (_type == _x || {_type isKindOf 'CAAnimalBase'}) exitWith {
					format['Запрещенный скин: %1', _type] call "+_kfc_ban+";
				};
			} count "+str _badSkins+";
			...
			player addEventHandler ['AnimChanged', {
				if ((_this select 1) in ['smk_urbanproneleft', 'smk_urbanproneright', 'smk_prone_to_urbanprone_left', 'smk_prone_to_urbanprone_right']) then {
					if (count(player nearEntities ['Plastic_Pole_EP1_DZ', DZE_PlotPole select 0]) > 0) then {
						...
						['Вы не можете выполнить эту анимацию рядом со Строй. столбом!', 4] call "+_kfc_msg+";
					};
				};
			}];
		...
		_queue set [count _queue, [{
			{
				if (currentWeapon player == _x) exitWith {
					format['Запрещенное оружие: %1', _x] call "+_kfc_ban+";
				};
...
_AH_Normal = _AH_Normal + ("
	...
	_queue set [count _queue, [{
		if (getPlayerUID player != '' && {name player != ("+_kfc_info+" select 0) || {getPlayerUID player != ("+_kfc_info+" select 1)}}) then {
			format['Информация об Игроке изменена! Текущая: %1 (%2) Должно быть: %3 (%4)', name player, getPlayerUID player, "+_kfc_info+" select 0, "+_kfc_info+" select 1] call "+_kfc_ban+";
		};
	}, 5.3, 0]];
");
...
_AH_Normal = _AH_Normal + ("
	_queue set [count _queue, [{
		if (typeName player != 'OBJECT') then {
			format['typeName игрока был изменен OBJECT на %1', typeName player] call "+_kfc_ban+";
		};
	...
	[] spawn {
...

		{
			if (str _x != str ("+_kfc_mic+" select _forEachIndex)) exitWith {
				...
				format['Файл миссии изменен: %1 - Содержимое: %2', _file, preprocessFile _file] call "+_kfc_ban+";
			};
		} forEach _counts;
	};
")};
...
local _AH_Admin = ("
	...
	AH_fnc_adjust = {
		...
		if (isNull _target) exitWith {['Цель не выбрана!', 2] call "+_kfc_msg+"};
		...
		[format['Вы изменили %1 из ""%2 (%3)"" на %4.', _adj, _name, _puid, [_qty] call BIS_fnc_numberText], 4] call "+_kfc_msg+";
		['Пожалуйста, подождите несколько секунд и обновите...', 4] call "+_kfc_msg+";
		format['Изменено %1 из ""%2 (%3)"" на %4', _adj, _name, _puid, [_qty] call BIS_fnc_numberText] call AH_fnc_adminLog;
	};
	...
	AH_fnc_cancelSpec = {
		...
		['Наблюдение окончено.', 4] call "+_kfc_msg+";
		'Прекратил наблюдение' call AH_fnc_adminLog;
	};
...
_AH_Admin = _AH_Admin + ("
	AH_fnc_initialize = {
		...
		local _rank = player call AH_fnc_rank;
		[format['Добро пожаловать %1! Загрузка Админ меню...', _rank], 2] call "+_kfc_msg+";
...
_AH_Admin = _AH_Admin + ("
		...
		"+_kfc_keys+" set [0x43, {
			if (admin_spectating) then {
				...
				format['Просмотр инвентаря ""%1 (%2)""', name admin_specTarget, getPlayerUID admin_specTarget] call AH_fnc_adminLog;
			};
		}];
		...
		['Админ меню загружено! Нажмите F2 чтобы открыть меню.', 2] call "+_kfc_msg+";
	};
");
_AH_Admin = _AH_Admin + ("
	AH_fnc_logs = {
	...
	AH_fnc_refresh = {
		...
		_ctrl ctrlSetText format['Игроков Онлайн: %1', count playableUnits];
	...
	AH_fnc_search = {
		...
		[format['Поиск всех элементов, содержащих ""%1"".', _find], 2] call "+_kfc_msg+";
	...
	AH_fnc_target = {
		...
		if (_this == 'Spectate') exitWith {
			if (isNil 'admin_specTarget') then {
				...
				"+_kfc_atrd_cfg+" set [count "+_kfc_atrd_cfg+", [{
					...
					[format[""<t size='0.5'>%1 (%2)</t><br/><t size='0.5'>Банк: %3 - Кошелек: %4 - Кровь: %5 - Человечность: %6</t><br/><t size='0.5'>Позиция: %7 - Оружие: %8</t>"",
			...
			if (_target == player) exitWith {if (!isNull admin_specTarget) then {call AH_fnc_cancelSpec} else {['Вы ни за кем не наблюдаете!', 2] call "+_kfc_msg+"}};
		...
		if (_this == 'Высадить Игрока с Техники') exitWith {
			...
			format['Высадка ""%1 (%2)"" из его Техники', _name, _puid] call AH_fnc_adminLog;
 		};
	};
	...
	AH_fnc_toggle = {
		...
		if (_this == 'Vehicle Boost') then {
			['Нажмите ""E"" для Ускорения и ""Space"" для Тормоза.', 4] call "+_kfc_msg+";
		};
	...
	admin_coords = {
		local _txt = format['Координаты: %1 - Направление: %2', [player] call FNC_getPos, getDir player];
	...
	admin_delete = {
		...
		if (isNull admin_cursorTarget) exitWith {['Цель не выбрана!', 2] call "+_kfc_msg+";};

		local _fnc_delete = {
			['KEY_DEL', admin_cursorTarget] call AH_fnc_adminReq;
			[format['Удаление ""%1"" успешно.', typeOf admin_cursorTarget], 4] call "+_kfc_msg+";
			format['Удаление ""%1"" @ %2', typeOf admin_cursorTarget, mapGridPosition player] call AH_fnc_adminLog;
		};

		[format['Вы уверены что хотите удалить ""%1""?', typeOf admin_cursorTarget], 4] call "+_kfc_msg+";

		admin_choice = [
			['', true],
			['Вы уверены?', [0], '', -5, [['expression', '']], '1', '0'],
			['Да', [2], '', -5, [['expression', format['call %1;', _fnc_delete]]], '1', '1'],
			['Нет', [3], '', -5, [['expression', '']], '1', '1']
		];
	...
	admin_fly = {
		...
		if (isNil 'admin_flycnt') then {
			...
			"+_kfc_atrd_cfg+" set [count "+_kfc_atrd_cfg+", [{
				...
				format['Взлетел вверх %1 раз за 30 секунд', admin_flycnt] call AH_fnc_adminLog;
				admin_flycnt = 0;
			}, 30, 0]];
		};
	...
	admin_forward = {
		...
		if (isNil 'admin_forwardcnt') then {
			...
			"+_kfc_atrd_cfg+" set [count "+_kfc_atrd_cfg+", [{
				...
				format['Телепортировался вперед %1 раз 30 секунд', [admin_forwardcnt] call BIS_fnc_numberText] call AH_fnc_adminLog;
				admin_forwardcnt = 0;
			}, 30, 0]];
		};
		...
		if (!isNull _ct && {player distance _ct < 10}) then {
			if (_ct isKindOf 'Air' || {_ct isKindOf 'LandVehicle'} || {_ct isKindOf 'Ship'}) then {
				...
				if (_charID != 0) then {
					...
					if (_isOk) then {
						[format['Вы добавили ""%1"" в свой инвентарь.', getText(configFile >> 'CfgWeapons' >> _key >> 'displayName')], 4] call "+_kfc_msg+";
						format['Добавлен ключ для ""%1"" в его инвентарь @ %2', typeOf _ct, mapGridPosition player] call AH_fnc_adminLog;
					} else {
						['У вас недостаточно места на поясе!', 2] call "+_kfc_msg+";
					};
				} else {
					['Вы не можете создать ключ для этой Техники.', 2] call "+_kfc_msg+";
				};
			};
		};
	};
	...
	admin_info = {
		local _ct = cursorTarget;
		if (isNull _ct) exitWith {['Цель не выбрана!', 2] call "+_kfc_msg+"};
		...
		local _txt = format['Имя: %1 ', if (isPlayer _ct) then {name _ct} else {_type}];
		if (_type in DZE_LockableStorage || {_type in DZE_DoorsLocked}) then {
			_txt = _txt + format['Комбинация: %1 ', _ct call _fnc_getCombo];
		} else {
			_txt = _txt + format['CharID: %1 ', _charID];
		};
		if (_coins > -1) then {_txt = _txt + format['Денег: %1 ', _coins]};
		...
		if ((_type isKindOf 'Air' || {_type isKindOf 'LandVehicle'} || {_type isKindOf 'Ship'}) && {_parse > 0}) then {
			...
			_txt = _txt + format['Ключ: %1 (%2) ', getText(configFile >> 'CfgWeapons' >> _key >> 'displayName'), _key];
		};
		if (count _friends > 0) then {
			...
			_txt = _txt + format['Друзей: %1 ', _friends];
		};
		_txt = _txt + format['Позиция: %1', [_ct] call FNC_getPos];

		[_txt, 5] call "+_kfc_msg+";
		format['Просмотр информации о ""%1"" @ %2', _type, mapGridPosition player] call AH_fnc_adminLog;
	};
	...
	admin_lock = {
		...
		if (!isNull _ct && {player distance _ct < 10}) then {
			if !(locked _ct) then {
				...
				[format['Вы закрыли ""%1"".', _type], 4] call "+_kfc_msg+";
				format['Закрыл ""%1"" @ %2', _type, mapGridPosition player] call AH_fnc_adminLog;
			} else {
				['Цель не открыта!', 2] call "+_kfc_msg+";
			};
		} else {
			['Цель не выбрана!', 2] call "+_kfc_msg+";
		};
	};
	...
	admin_spawnBp = {
		...
		if (admin_targetSpawn) then {
			...
			[format['Вы выдали Рюкзак ""%1"" к ""%2 (%3).""', _this, name admin_curTarget, getPlayerUID admin_curTarget], 4] call "+_kfc_msg+";
			format['Выдан Рюкзак ""%1"" к ""%2 (%3)""', _this, name admin_curTarget, getPlayerUID admin_curTarget] call AH_fnc_adminLog;
		} else {
			player addBackpack _this;
			call player_forceSave;
			[format['Вы добавили ""%1"" к себе заспину.', _this], 4] call "+_kfc_msg+";
			format['Выдан Рюкзак ""%1"" @ %2', _this, mapGridPosition player] call AH_fnc_adminLog;
		};
	};
	...
	admin_spawnEvent = {
		...
		[format['Вы запустили Ивент: ""%1.""', _this], 4] call "+_kfc_msg+";
		format['Запущен Ивент: ""%1"" @ %2', _this, mapGridPosition player] call AH_fnc_adminLog;
	};

	admin_spawnMag = {
		if (admin_targetSpawn && {isNull admin_curTarget}) exitWith {['Цель не выбрана!', 2] call "+_kfc_msg+"};
		['Spawning Magazine', 'Количество:', format[""
			local _qty = parseNumber(ctrlText 1400);
			...
			if (admin_targetSpawn) then {
				...
				['Вы выдали '+str _qty+' '+str %1+' к '+str admin_curTarget+'.', 4] call "+_kfc_msg+";
				'Выдано '+str _qty+' '+str %1+' к '+str admin_curTarget+' @ '+mapGridPosition player+'' call AH_fnc_adminLog;
				admin_curTarget = nil;
			} else {
				for '_x' from 1 to _qty do {player addMagazine %1};
				call player_forceSave;
				['Добавлен '+str _qty+' '+str %1+' в ваш Инвентарь.', 4] call "+_kfc_msg+";
				'Добавлено '+str _qty+' '+str %1+' в Инвентарь @ '+mapGridPosition player+'' call AH_fnc_adminLog;
			};
	...
	admin_spawnVeh = {
		if (vehicle player != player) exitWith {['Вы не можете создать технику пока находитесь в технике.', 2] call "+_kfc_msg+";};
	...
	admin_time = {
		...
		[format['Вы установили время на сервере: %1', _this], 4] call "+_kfc_msg+";
		format['Установлено время на сервере: %1', _this] call AH_fnc_adminLog;
	};
	...
	admin_unlock = {
		...
		if (!isNull _ct && {player distance _ct < 10}) then {
			if (locked _ct || {_type in (DZE_LockedStorage + DZE_DoorsLocked)}) then {
				...
				[format['Вы открыли ""%1"".', _type], 4] call "+_kfc_msg+";
				format['Открыл ""%1"" @ %2', _type, mapGridPosition player] call AH_fnc_adminLog;
			} else {
				['Цель не закрыта!', 2] call "+_kfc_msg+";
			};
		} else {
			['Цель не выбрана!', 2] call "+_kfc_msg+";
		};
	...
	admin_weather = {
		...
		[format['Вы изменили погоду на сервере: %1.', toLower _this], 4] call "+_kfc_msg+";
		format['Изменена погода на сервере: %1', toLower _this] call AH_fnc_adminLog;
	};
...
_AH_Server = _AH_Server + ("
	...
	AH_fnc_deleteObj = {
		...
		['SERVER', 1, format['Объект удален %1', if (parseNumber _id > 0) then {format['ID: %1', _id]} else {format['UID: %1', _uid]}]] call AH_fnc_log;
	};
	...
	AH_fnc_log = {
		...
		if (_opt > 1) then {
			...
			'Antihack' callExtension format['Log|%1|%2%3', _type, _log, format[' | Uptime: %1 Карта: %2-%3', call AH_fnc_uptime, dayZ_instance, worldName]];
		};
	};
	...
	AH_fnc_ban = {
		...
		if (count _this < 4) then {
			_why = format['АнтиЧит Авто-Бан ID: %1', _id];
		} else {
			_dur = _this select 3;
		...
		_log = format['Блокировка (ID: %1) за: %2', _id, _log];
	...
	"+_kfc_strd_cfg+" set [count "+_kfc_strd_cfg+", [{
		['SERVER', 1, format['FPS: %1 | ИИ: %2 | Игроков: %3 | Uptime: %4',
			round diag_fps,
...
_AH_Server = _AH_Server + ("
	AH_fnc_procAdminReq = {
		...
		local _fnc_sendRE = {
			...
			['SERVER', 1, format['RE запрос отправлен игроку ""%1 (%2)""', name _target, getPlayerUID _target]] call AH_fnc_log;
		};
		...
		if (_id == 'TOOL_KILLAI') exitWith {
			...
			[_pobj, 2, format['Удалено %1 ИИ в пределах %2 метрах от %3', _cnt, _param, mapGridPosition _pobj]] call AH_fnc_log;
		};
		if (_id == 'MENU_ADJUST') exitWith {[_pobj, _param, _id] call _fnc_sendRE};
		if (_id == 'VEH_TMPSPAWN') exitWith {
			comment 'Создает временную технику (без Сохранения в БД)';
		...
		if (_id == 'MENU_CODE') exitWith {
			if ((_param select 1) == 1) then {
				...
				[_pobj, 2, format['Выполнен следующий код на Сервере: %1', toString(_param select 0)]] call AH_fnc_log;
			} else {
				...
				[_pobj, 2, format['Выполнен следующий код Глобально для всех: %1', toString(_param select 0)]] call AH_fnc_log;
			};
		};
...
if (_safezones) then {
	if (_bubbles) then {
		_AH_Server = _AH_Server + ("
			...
			['SERVER', 1, format['Создано %1 ограничителей для Торговой зоны', _cnt]] call AH_fnc_log;
		");
	};
	if (_antized) then {
		_AH_Server = _AH_Server + ("
			"+_kfc_strd_cfg+" set [count "+_kfc_strd_cfg+", [{
				...
				if (_cnt < 1) exitWith {};
				['SERVER', 1, format['Удалено %1 Зомби из Торговых зон', _cnt]] call AH_fnc_log;
			}, 3.3, 0]];
		");
	};
	if (_antiai) then {
		_AH_Server = _AH_Server + ("
			"+_kfc_strd_cfg+" set [count "+_kfc_strd_cfg+", [{
				...
				if (_cnt < 1) exitWith {};
				['SERVER', 1, format['Удалено %1 ИИ из Торговых зон', _cnt]] call AH_fnc_log;
			}, 3.3, 0]];
		");
	};
};
	...
	_AH_Server = _AH_Server + ("
		AH_fnc_vote = {
			...
			if (_time < "+str _votedelay+") exitWith {
				[_pobj, format['Подождите еще %1 секунд для проведения голосования.', round("+str _votedelay+" - _time)]] call AH_fnc_msg;
			};
			...
			if (isNil 'AH_voteInit') then {
				...
				"+_kfc_strd_cfg+" set [count "+_kfc_strd_cfg+", [{
					...
					if (diag_tickTime - AH_voteInit >= "+str _votetime+") then {
						['All', 'Достигнут лимит времени голосования - необходимое количество голосов не достигнуто.'] call AH_fnc_msg;
			...
			if (AH_voteInit < 0) then {
				...
				['All', 'Голосование за День/Ночь было начато!'] call AH_fnc_msg;
			};

			if (isNil 'AH_votes') then {AH_votes = []};
			if (_puid in AH_votes) exitWith {
				[_pobj, 'Вы уже проголосовали!'] call AH_fnc_msg;
			};

			{AH_votes set [count AH_votes, _x]} count [_vote, _puid];

			[_pobj, 9, format['Вы отдали голос за: %1', if (_vote == 0) then {'Ночь'} else {'День'}]] call AH_fnc_log;
			...
			['All', format['Текущий подсчет голосов: %1 за День | %2 за Ночь', _day, _night]] call AH_fnc_msg;

			if (_total < _needed) exitWith {};

			['All', format['Подсчет голосов окончен! Результаты: %1 за День | %2 за Ночь', _day, _night]] call AH_fnc_msg;
			...
			local _hour = call {
				...
				if (_day == _night) exitWith {
					...
					['All', format['Голоса разделились поровну - Сервер выбрал: %1.', if (_rnd > 0) then {'День'} else {'Ночь'}]] call AH_fnc_msg;
					_rnd
				};
			};
...
_AH_Server = _AH_Server + ("
	[] spawn {
		...
		server_tradeObj = {
			...
			local _log = format['%1 %2 ""%3"" %4 %5 @ %6 за %7', if (_bors == 0) then [{'Купил'}, {'Продал'}], _qty, _this select 3, if (_bors == 0) then [{'в'}, {'из'}], _cont, _this select 4, _cost];
		...
		server_handleSafeGear = {
			...
			local _log = format['%1 Тип: %2 на Локации @ %3 и Хозяин UID: %4 - Правильный код: %5',
				...
				_code,
				if (count _this > 3) then {format[' - Код получен: %1', _sent]}
			];
		...
		server_playerDied = {
			...
			['', 11, format['%1 (%2) умер от %3', _pname, _this select 3, mapGridPosition (getPosATL (_this select 2))]] call AH_fnc_log;
		...
		if (isNil 'serverTraders') exitWith {
			['SERVER', 1, 'Функция Проверки техники не сработала! Переменная serverTraders не существует! Файл торговли перемещен!'] call AH_fnc_log;
		};
		...
		local _fn_traderVehs = {
			local _arr = [];
			{
				...
				if (!isNil _cat) then {
					...
					['SERVER', 1, format['NPC класс ""%1"" не имеет привязанного Меню Торговли. Удалите его в массиве serverTraders в папке с миссией.', _x]] call AH_fnc_log;
				};
			} forEach serverTraders;
			...
			local _arr = entities 'Air' + entities 'LandVehicle' + entities 'Ship';
			{
				...
				if (!(toLower _type in AH_allowedVehs) && {!(toLower(getText(configFile >> 'CfgVehicles' >> _type >> 'original')) in AH_allowedVehs)} && {!(_type in DZE_safeVehicle)} && {!(_type isKindOf 'StaticWeapon')} && {!(['Parachute', _type] call fnc_inString)}) then {
					local _veh = _x; local _plrs = [];
					{
						...
						if (_dis < 100) then {
							_plrs set [count _plrs, format['%1 (%2) [%3м]', name _x, getPlayerUID _x, round _dis]];
						};
					} count playableUnits;

					['SERVER', 3, format['Удалена Неразрешенная техника ""%1"" @ %2 - Игроки рядом: %3', _type, mapGridPosition _veh, _plrs]] call AH_fnc_log;
					[_veh, true] call AH_fnc_deleteObj; comment 'Код должен считываться ниже журнала, чтобы мы могли получить mapGridPosition техники, пока она существует.';
				};
			} count _arr;
...
if (_mic) then {_AH_Server = _AH_Server + ("
	[] spawn {
		local _counts = [];
		['SERVER', 1, 'Обработка файлов миссии...'] call AH_fnc_log;
		{
			local _arr = []; for '_i' from 0 to 128 do {_arr set [_i, 0]};
			local _file = toArray(preprocessFile _x);
		...
		['SERVER', 1, 'Файлы миссии успешно обработаны!'] call AH_fnc_log;
...
_AH_Server = _AH_Server + ("
	...
	"+_kfc_strd_cfg+" set [count "+_kfc_strd_cfg+", [{
		...
		call "+_kfc_re+";
		['SERVER', 3, 'АнтиЧит функция RE была удалена Читером! Новая функция RE была создана. Исправьте фильтры BE, чтобы предотвратить взлом!'] call AH_fnc_log;
	}, 1, 0]];
");
