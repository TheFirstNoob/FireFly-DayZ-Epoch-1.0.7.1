...
[
	8, 		// Мин. время (в Минутах) для изменения погоды. (По умолчанию: 15).
	30, 	// Макс. время (в Минутах) для изменения погоды. (По умолчанию: 30).
	0, 		// Мин. интенсивность тумана (0 = Нет тумана, 1 = Макс. туман). (По умолчанию: 0).
	.6, 	// Макс. интенсивность тумана (0 = Нет тумана, 1 = Макс. туман). (По умолчанию: 0.8).
	...
	1, 		// Макс. интенсивность облачности (0 = Чистое небо, 1 = полностью затянуто облаками). (По умолчанию: 1).
	0, 		// Мин. интенсивность дождя (0 = Нет дождя, 1 = Макс. дождя). Чтобы пошел дождь, облачность должна быть не менее 70%.
	0, 		// Макс. интенсивность дождя (0 = Нет дождя, 1 = Макс. дождя). Чтобы пошел дождь, облачность должна быть не менее 70%.
	0, 		// Мин. сила ветра (По умолчанию: 0).
	7, 		// Макс. сила ветра (По умолчанию: 5).
	.30, 	// Вероятность смены ветра при изменении погоды. (По умолчанию: .25).
	.2, 	// Мин. интенсивность снега (0 = Нет снега, 1 = Макс. снег). Чтобы пошел снег, облачность должна быть не менее 75%.
	1, 		// Макс. интенсивность снега (0 = Нет снега, 1 = Макс. снег). Чтобы пошел снег, облачность должна быть не менее 75%.
	.5, 	// Вероятность метели во время снегопада. (0 = Нет метели, 1 = Всегда метель). (По умолчанию: .2).
	...
	150, 	// Расстояние (в Метрах) от игрока, чтобы сканировать здания для создания тумана. По умолчанию только 15 ближайших зданий будут создавать туман.
...
if (isServer) then
{
	...
	if (DZE_EVR) then
	{
		DZE_EVRFirstTime 	= 	[60, 240]; 	// Случайное время когда будет первый ПСИ-Шторм после запуска сервера. [Мин, Макс] (В минутах).
		DZE_EVRTimer 		= 	[60, 180]; 	// Случайное время когда будет следующий ПСИ-Шторм. [Мин, Макс] (В минутах).
	};
};
...
if (!isDedicated) then
{
	...
	dayz_randomMaxFuelAmount 	= 	500; 			// Добавляет случайное количество топлива на все заправочные станции (В литрах).
	...
	DZE_CombatTimer 			= 	30; 			// Время "В бою" после выстрела или активного действия (В секундах).
	...
	DZE_ZombieSpeed 			= 	[0,0]; 			// Стандартная скорость Агрессии зомби 6 на каждого зомби, установите элементы массива 0 и 1 одинаковыми для непеременной скорости, Установите 0 чтобы отключить. Формат = [Мин, Макс];  Например: [2, 6]; приводит к диапазону скоростей от 2 до 6 (2 — старая жестко запрограммированная скорость DZE_slowZombies)
	...
	*/
	...
	[
		...
		, 3			// На Солнце
		, 12		// Использование Хим.Грелки
		, 9			// Одет "Утепленный" скин
		
		// Отрицательное значение
		, 6			// Вода, Море и т.п.
		, 4			// Игрок не передвигается
		, 0.75		// Идет Дождь
		, 0.55		// Ветер
		, 0.7		// Ночь
		, 12		// Снег
	...
	DZE_SafeZoneNoBuildDistance		=	750; 		// Дистанция для объектов возле Трейд-Зон DZE_SafeZonePosArray.
